...
export const EditSportPage = ({ record }) => {
    const history = useHistory();
    const [formvalue, setFormvalue] = useState(initialFormValue);
    const [errors, setErrors] = useState({});
    const location = useLocation();
    const sportsData = location.state.record || {};
    ...
    const handleSubmit = async () => {
        try {
            ...
            let { status, loading, error, message } = await EditSports(passData);
            if (status) {
            ...
            <div className="common_page_scroller pb-5 mt-3 mt-sm-5 pe-2">
                <section className="editPageContainer">
                    <div className="cmn_modal_header d-flex justify-content-between align-items-center">
                        <p className="cmn_modal_title">Edit Sports</p>
